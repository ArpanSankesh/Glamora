...
    setItems((prev) => [
      ...prev,
      { name: "", price: "", duration: "", imageFile: null, preview: EMPTY_PREVIEW },
    ]);
...
        (it) => !it.name || !it.price || !it.duration || !it.imageFile
      );
      if (invalidItem) {
        setMessage("Every service must have a name, price, duration, and image.");
        return;
      }
    }
...
            price: Number(items[i].price),
            duration: Number(items[i].duration),
            imageUrl: url,
          });
        }
      }
