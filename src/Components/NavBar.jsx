// SOLUTION 1: Add padding-top to body/main content areas
// Add this to your main CSS file or component styles
const globalStyles = `
  body {
    padding-top: 110px; /* Match your navbar height */
  }
  
  /* Or apply to your main content wrapper */
  .main-content {
    padding-top: 110px;
  }
`;

// SOLUTION 2: Create a spacer component
const NavbarSpacer = () => {
  return <div className="h-[110px]" />; // Match navbar height
};
...
// SOLUTION 3: Modified NavBar with dynamic positioning
...
const NavBar = () => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 w-full h-[110px] transition-all duration-300
      ...
      >
        <div className='w-full flex items-center justify-between flex-col'>
          ...
          <div className='w-full flex items-center justify-between mt-3 px-6 md:px-16 lg:px-24 '>
            {/* Logo */}
            ...
            {/* Mobile Menu */}
            <div className={`fixed top-[110px] left-0 w-full bg-white shadow-lg z-10 transition-all duration-300 md:hidden ${isMobileMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4 pointer-events-none'}`}>
              <div className="p-6">
      ...
      {/* SOLUTION 4: Spacer div that appears on all pages except home */}
      {!isHomePage && <div className="h-[110px]" />}
    </>
  );
};
...
// SOLUTION 5: Layout wrapper component
const PageLayout = ({ children, className = "" }) => {
  const location = useLocation();
  const isHomePage = location.pathname === '/';
  
  return (
    <div className={`${!isHomePage ? 'pt-[110px]' : ''} ${className}`}>
      {children}
    </div>
  );
};
